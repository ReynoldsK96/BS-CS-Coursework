...
public class Lab6 {
   ...
   private static boolean problem1(Node t1, Node t2) {
      ...
      // if values are not equal then the trees are not the same and it returns false
      if (t1.value != t2.value) {
         return false;
      }
      
      // catches if trees are not the same in anyway i.e. if there is a t1.left but not a t2.left then they are not the same and it returns false
      ...
      if (t1.value == t2.value) {
         ...
         // if there are nodes on both sides of the current node
         if (((t1.left != null) && (t1.right != null)) && ((t2.left != null) && (t2.right != null))) {
            // recursively goes left
            status = problem1(t1.left, t2.left);
            // if it returns false from the recusion
            if (!status) {
               return false;
            }
            // recursively goes right
            status = problem1(t1.right, t2.right);
            // if it returns false from the recursion 
            if (!status) {
               return false;
            }
         }
         
         // if there is only a node of the left side of the current node
         if (((t1.left != null) && (t1.right == null)) && ((t2.left != null) && (t2.right == null))) {
            // recursively goes left
            status = problem1(t1.left, t2.left);
            // if it returns false from the recursion 
         ...
         // if there is only a node on the right side of the current node
         if (((t1.left == null) && (t1.right != null)) && ((t2.left == null) && (t2.right != null))) {
            // recursively goes right 
            status = problem1(t1.right, t2.right);
            // if it returns false from the recursion
      ...
      // if the method makes it all the way to this return statement then the trees are the same
      return true;
   }
   ...
   private static Node problem2(Node t1, Node t2) {
      ...
      // adds t2.value to t1.value to combine the two values
      t1.value += t2.value;
