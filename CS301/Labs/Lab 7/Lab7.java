...
public class Lab7 {
   ...
   private static boolean problem1(Node t1, Node t2) {
      ...
      // creates arrays out of the binary trees, from min to max values
      int[] arr1 = getPostOrder(t1);
      int[] arr2 = getPostOrder(t2);
      
      // checks to see if the arrays are the same length, if they are not then they do not have the same number of nodes and they do not store the same numbers
      if (arr1.length != arr2.length) {
         return false;
      }
      
      // checks to see if the array values are the same, if the arrays don't match then the trees do not store the same numbers
      ...
      // if the methods reaches this point then the trees store the same numbers
      return true;
   }
   ...
   private static int problem2(Node root, int min, int max) {
      ...
      // recursively go to the left of the tree and store keys found to be within the given range
      int left = problem2(root.left, min, max);
      
      // if the key is within the given range 
      if ((root.key >= min) && (root.key <= max)) {
         // return the key within the given range and recursively search through the tree for more keys
         return root.key + problem2(root.left, min, max) + problem2(root.right, min, max);
      }
      
      // recursively go to the right of the tree and store keys found to be within the given range
      int right = problem2(root.right, min, max);
      
      // add the left and right sums together to get the total sum of keys found within the given range 
      sum = left + right;
