...
public class Lab2
{
   ...
   private static boolean problem1(int[] arr, int i, int j) {         
      ...
      if (i == j) {
         return false;
      // tests to see if i and the number that directly follow it are equal, if so the method returns true
      } else if (arr[i] == arr[i+1]) {
         return true;
      // recursively calls the method and adds 1 to the value of i
      } else {
   ...
   private static boolean problem2(int[] arr, int i, int j) {
      ...
      if (i >= j) {
         return true;
      // tests to see if arr value at index i and j are equal, if so method is called recursively, increasing i by 1 and decreasing j by 1
      } else if (arr[i] == arr[j]) {
         return problem2(arr, ++i, --j);
      // second base case
      } else {
         return false;
      }
